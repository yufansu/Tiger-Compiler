...
static struct expty transExp(Tr_level level, Tr_exp breakk, S_table venv, S_table tenv, A_exp a)
{
    ...
    case A_callExp:
    {
        A_expList args = NULL;
        Ty_tyList formals;
        E_enventry x = S_look(venv, a->u.call.func);//To see whether the function is defined
        Tr_exp TRanslation = Tr_noExp();
        Tr_expList argList = Tr_ExpList();
        if (x && x->kind == E_funEntry) {
            formals = x->u.fun.formals;
            int count = 0;
	    //check types of the function parameters
	...
	//There is no return check. The return parameter is in the last line of the fuction
    }
    case A_opExp:
    {
        ...
        Tr_exp TRanslation = Tr_noExp();
        switch (oper) {
        //+ - * / are all the same We need integers.
        case A_plusOp:
        case A_minusOp:
        case A_timesOp:
        case A_divideOp:
	...
	// = <> are the same. And the check is also the same as eqExp
	// We need to check both string and int.
        case A_eqOp:
        case A_neqOp:
            switch(left.ty->kind) {
            case Ty_int:
...
static Ty_ty transTy(S_table tenv, A_ty t)
{
    ...
    //Record Type check is just field type check. So make a fieldType and have a check
    case A_recordTy:
    {
...
static Ty_ty actual_ty(Ty_ty ty)
{
    //naive function. If there is a muti-link type definition, the function fails.
    if (ty->kind == Ty_name)
